  ...
  -
    ...
    ControllerConfig:
      ...
      REALITYConfigs:
        ...
        Dest: 1.1.1.1:443 # Required, Same as fallback
      ...
      CertConfig:
        ...
        CertFile: /etc/XrayR/quoctai.crt # Provided if the CertMode is file
        KeyFile: /etc/XrayR/quoctai.key
...
case "${num}" in
1) bash <(curl -Ls https://raw.githubusercontent.com/dongbao492/script_xrayr/master/install.sh)
openssl req -newkey rsa:2048 -x509 -sha256 -days 365 -nodes -out /etc/XrayR/quoctai.crt -keyout /etc/XrayR/quoctai.key -subj "/C=JP/ST=Tokyo/L=Chiyoda-ku/O=Google Trust Services LLC/CN=google.com"
cd /etc/XrayR
  cat >config.yml <<EOF
Log:
